...
namespace SudokuSolver.Strategies
{
    public class SimpleMarkUpStrategy : ISudokuStrategy
    {
            ...
            for(int row = 0; row < sudokuBoard.GetLength(0); row++)
            {
                for(int col = 0; col < sudokuBoard.GetLength(1); col++)
                {
                    if(sudokuBoard[row, col] == 0 || sudokuBoard[row, col].ToString().Length > 1)
                    {
                        ...
                        var possibititiesInBlock = GetPossibitiesInBlock(sudokuBoard, row, col);
                        sudokuBoard[row, col] = GetPossibilityIntersection(possibilitiesInRowAndCol, possibititiesInBlock);
                    }
                }
            }
        ...
        private int GetPossibitiesInBlock(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            int[] possibilities = { 1, 2, 3, 4, 5, 6, 7, 8, 9 };
        ...
        private int GetPossibilityIntersection(int possibilitiesInRowAndCol, int possibititiesInBlock)
        {
            var possibilitiesInRowAndColCharArray = possibilitiesInRowAndCol.ToString().ToCharArray();
            var possibilitiesInBlockCharArray = possibititiesInBlock.ToString().ToCharArray();
